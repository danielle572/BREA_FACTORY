...
**Version:** 1.6 | **Session:** Factory Session F8 — CLI Execution Confirmed + Log Access + Voice Stop
...
**Brea 1 — The Live B2B Prototype (BEOS)**
- Role: Live proof-of-concept for Boujie Girl Specialty Beauty Boutique
- Deployment: Cloud-hosted, voice + kiosk + web URL
- Capabilities: Inbound calls, booking, FAQ, service availability
- Airtable Base: BEOS Base (separate, isolated)
- Status: Active build, checkout fixes pending
...
- Airtable Base: FACTORY Base (needs to be created — see Part 6)
...
| 094 | Commission Split Logic | Pending Build | BEOS + Tenants | T3 | — |
| 095 | Checkout Flow | Pending Build — BEOS | BEOS | T1 | — |
| 096 | Gift Card Purchase + Redemption | Pending Build — BEOS | BEOS | T1 | — |
...
Assign the next available ID number (currently at **188**).
...
| 167 | Auto Task Execution — CONFIRMED WORKING | Agentic Pipeline | shell=True + stdin + scoped allowlist. Test comment written to dashboard/app.py verified. | ✅ Done Session F8 |
| 182 | Chat Persistence — CONVERSATION_LOG Airtable | Factory Voice | CONVERSATION_LOG table + Airtable write on each message | Session F6 (priority 2) |
| 146 | Fix Deepgram 400 — audio format mismatch | Factory Voice | Confirm content-type accepted by Deepgram for Pixel audio | Session F6 (priority 3) |
| 183-185 | Two-mode mic, replay button, pause/stop | Factory Voice | Deepgram fix must land first | Session F6 |
| 186-187 | Unified Portal — Brea 3 + Factory + BOSS | Unified Portal | Factory voice stable, CLI fix done | Session F7 |

---

## PART 9 — IMMEDIATE NEXT STEPS IN ORDER

**Step 1 — Verify Stop button + /logs in browser (15 min)**
...
**Step 2 — Queue first real build task via Brea**
...
**Step 3 — BREA_WEBAPP cleanup (next Brea 3 session)**
See the separate BREA_WEBAPP_HANDOFF_NOTE.md for exact instructions.

**Step 4 — Technical Support Brea spec**
...
**Step 5 — Screen Presence / Assisted Control (Session SC1)**
Spec complete (IDs 172-178). Build when Factory is fully stable.

---

**How to start next Brea 3 session:**
```
Read BREA_EMPIRE_MASTER_KNOWLEDGE_BASE.md and BREA_WEBAPP_HANDOFF_NOTE.md from project files.
We are cleaning up the BREA_WEBAPP session from June 10.
Start by confirming what was written to disk — read app.py, voice-session.js, chat.js, and static/readback.js.
Do not change anything yet.
```

Priority 1 — Fix Auto Task Execution (30 min)
- Locate claude.exe on Aurora (`where claude` in terminal)
- Update brea_factory.py executor with absolute path
- Test one Auto task end to end
- This is the last wire — without it nothing Brea queues actually runs

Priority 2 — Fix chat persistence (1 hour)
- Create CONVERSATION_LOG table in BREA_FACTORY base if not present
- Save each message to Airtable on send
- Load last 20 messages on page start
- Messages survive refresh and Aurora restart

Priority 3 — Fix Deepgram 400 audio format error (30 min)
- Read on_voice_stop function as it actually exists in app.py
- Confirm content-type retry loop is actually in the file (edits may not have saved due to watchdog timing)
- Test audio/webm as primary content type for Pixel MediaRecorder output
- Confirm [DEEPGRAM] Success log line appears in terminal

Priority 4 — Fix Health Log FAIL (5 min)
- Remove Pattern_Flag from health diagnostic write payload in brea_factory.py
- Silent error, not blocking, but cleanup is fast

Priority 5 — Add replay button and pause/stop (once voice stable)
- Per-message ▶ replay stored from audio_chunk done:true
- Pause: audioCtx.suspend/resume
- Stop: voice_interrupt socket event

**Step 2 — Session F7: Unified Portal**
- Brea 3 Business Tab reads Factory status and BOSS status
- Factory panel embeddable inside Brea 3
- One portal for everything
- Prerequisite: Session F6 complete (voice stable + CLI executing)

**Step 3 — Session D: Document Scanner (Brea 3)**
- jscanify + Claude Vision + Pristine Asset Tracker
- ANTHROPIC_API_KEY confirmed present in BREA_WEBAPP/.env

**Step 4 — Session D.6: ReadBack / App Widget (Brea 3)**
- NOTES table confirmed live — fully unblocked

**Step 5 — Sessions N1 and N2 (Brea 3)**
- N1: Persistent Notification Loop (IDs 121-126)
- N2: Dashboard Awareness Layer (IDs 127-131)
...
*End of BREA_EMPIRE_MASTER_KNOWLEDGE_BASE v1.6*
*Updated: Factory Session F8 — June 11, 2026*
*Changes: CLI execution confirmed working (ID 167 Active), Stop Session button live (ID 185), orchestrator log tee added (ID 196), /logs route live (ID 197), Brea real log access in diagnosis context (ID 198), Part 9 next steps updated, next available ID updated to 199*
