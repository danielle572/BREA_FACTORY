...
def on_voice_stop():
    ...
    print(f"[DEEPGRAM] Sending {len(audio_data)} bytes")
    ...
    for ct in content_types:
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"[DEEPGRAM] Success ({ct}): {transcript}")
                break
            else:
                print(f"[DEEPGRAM] Rejected ({ct}): {resp.text[:300]}")
        except Exception as e:
            print(f"[DEEPGRAM] Exception ({ct}): {e}")
            traceback.print_exc()
            continue
...
def _respond_from_voice(transcript: str, sid: str) -> None:
    import re
